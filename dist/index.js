...
function extractToken(input) {
    ...
    if (!match)
        throw new Error("No valid 64-char token found in input");
...
async function encryptContent(content, secret) {
    const keyMat = await crypto.subtle.digest("SHA-256", hexToBytes(secret).buffer);
    const key = await crypto.subtle.importKey("raw", keyMat, { name: "AES-GCM", length: 256 }, false, ["encrypt"]);
 ...
 */
export async function read(urlOrToken, base = DEFAULT_BASE) {
 ...
 * @param base      Override the base URL (default: https://voidnote.net)
 */
export async function create(content, options, base = DEFAULT_BASE) {
